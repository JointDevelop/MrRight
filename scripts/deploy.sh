...
install_redis() {
    ...
    then
        wget -P /tmp/ 'http://download.redis.io/releases/redis-6.2.1.tar.gz'
        tar -xzf /tmp/redis-6.2.1.tar.gz -C /tmp
        cd /tmp/redis-6.2.1
        make && make install
        cd -
        rm -rf /tmp/redis*
...
project_init() {
    ...
    pip install -U pip
    if [ -f $proj/requires.txt ]; then
        pip install -r $proj/requires.txt
    fi
    deactivate
